...
// ── Refresh token queue ──────────────────────────────────────────────────────
// Handles concurrent requests that arrive while a refresh is in-flight.
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      // Queue any additional 401s that arrive while a refresh is already in-flight
      if (isRefreshing) {
...
export const pdmAnualService = {
  ...
  getComparativo: async () => {
    const response = await api.get('/pdm/anual/comparativo');
    return response.data;
  },
...
export const zonaRuralService = {
  ...
  getStats: async (vereda = null) => {
    const params = vereda && vereda !== 'TODAS' ? { vereda } : {};
    const res = await api.get('/zonarural/stats', { params });
    return res.data;
  },
  getBreakdown: async () => {
    const res = await api.get('/zonarural/breakdown');
    return res.data;
  },
};
